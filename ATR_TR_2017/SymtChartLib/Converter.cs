...
{
    public class TextBlockConverter : MarkupExtension, IValueConverter
    {
    ...
    public class FreqStringConverter : MarkupExtension, IValueConverter
    {
        ...
        public object Convert(object value, Type targetType, object parameter, System.Globalization.CultureInfo culture)
        {
            try
            {
                ...
                {
                    int n = (int)Math.Floor(Math.Log10(freq) / 3);
    ...
    public class TagConverter : MarkupExtension, IValueConverter
    {
        ...
        public object Convert(object value, Type targetType, object parameter, System.Globalization.CultureInfo culture)
        {
            ...
            if (!string.IsNullOrWhiteSpace(str))
            {
                ...
                string result1 = System.Convert.ToString((new SIPrefixConverter()).Convert(strs[0], null, null, null));
                double? tempValue = strs[1].ToString().ToNullDouble();
                if (tempValue != null)
                {
                    return result1 + "~" + System.Convert.ToDouble(tempValue).ToDigits(GeneTestSetup.Instance.DataDisplayDigits);
                }
                else
                {
                    return result1;
    ...
    //public class DatadigitalConverter : MarkupExtension, IValueConverter
    //{
    //    public override object ProvideValue(IServiceProvider serviceProvider)
    //    {
    //        return this;
